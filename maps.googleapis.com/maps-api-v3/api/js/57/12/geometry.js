google.maps.__gjsload__("geometry", function (_) {
  var Hpa = function (a, b) {
    ...
    Ipa = function (a, b, c, d, e) {
      if (!d) {
        ...
        if (!e)
          return (
            (e = Math.sin(_.Uf(a.lat()))),
            (e = Math.log((1 + e) / (1 - e)) / 2),
            (b = Math.sin(_.Uf(b.lat()))),
            _.Vf(
              2 *
                Math.atan(
        ...
        return e
          .fromPointToLatLng(
            new _.Zl(a.x + c * (b.x - a.x), a.y + c * (b.y - a.y))
          )
          .lat();
      }
      e = _.Uf(a.lat());
  ...
  QB.isLocationOnEdge = function (a, b, c) {
    ...
    for (let t = 0, v = g.getLength(); t < v; ++t) {
      ...
      for (let B = 0; B < z; ++B) {
        ...
        if (
          (k =
            Math.abs(_.mj(k - m, -180, 180)) <= 1e-9 &&
            (Math.abs(_.mj(k - d, -180, 180)) <= c ||
              Math.abs(_.mj(m - d, -180, 180)) <= c))
        ) {
          k = a.lat();
  ...
  var Lpa = { encoding: PB, spherical: _.Aq, poly: QB };
